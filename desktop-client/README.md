...
- 也可以从列表里手动选择某个客户任务，或直接选择“使用（可分配）”的 SIM 激活码自动创建测试任务，方便测试和容错
- 显示并复制邮箱、初始密码、SIM 激活码、地址
- 刷新 MoEmail 验证码并复制到剪贴板
- 打开 Playwright 浏览器并尝试预填 giffgaff 页面
- 页面加载慢或中途中断时，可点击“继续当前页面”从当前 giffgaff 页面继续识别并执行下一步
- 可配置“页面超时 ms”和“单步等待 ms”，默认分别为 `120000` 和 `15000`
- 固定选择 Pay as you go、£10 top-up，并自动填写本机预设的英国地址和支付卡
- 支付完成后自动打开支付方式页并移除已保存信用卡，随后通过 MoEmail 检查 giffgaff 的 `your payment info has changed` 邮件确认已解绑
...
6. 点击“领取下一个任务”；如果要测试指定激活码，点击“选择激活码任务”。列表会同时显示已有客户任务，以及 SIM 激活码页面里“使用（可分配）”的未分配激活码。选择未分配激活码时，后台会自动创建 MoEmail 客户任务并立即领取。
7. 在“激活自动化预设”里填好固定英国地址、支付卡和“支付后自动解绑银行卡”。
8. 点击“打开并预填”，浏览器会打开 giffgaff 激活页。
9. 遇到邮箱验证码时点击“刷新验证码”，客户端会复制验证码；浏览器自动化还在运行时可点击“填入验证码”。
10. 如果 giffgaff 页面加载慢、卡住、人工点过下一步，等待页面显示稳定后点击“继续当前页面”；客户端会根据当前页面自动继续填写/点击。
11. 客户端会继续完成密码、营销偏好、Pay as you go、£10、地址和支付卡填写。
12. 到支付页后，人工确认并点击 `Place order`，完成支付或 3DS。
13. 支付完成后保持浏览器打开；客户端会自动进入 `/profile/payment-details` 并移除保存的信用卡。也可以点击“自动解绑银行卡”手动触发。解绑后客户端会等待 MoEmail 里出现 `your payment info has changed` 邮件；`your payment info has been updated` 只代表支付信息已更新/绑卡，不代表已解绑。
14. 拿到手机号后填入客户端，点击“标记等待转 eSIM”或“标记完成”。

领取任务时，后台会自动跳过已经填写手机号的旧客户，避免把已手工处理过的客户再次交给客户端。
手动选择任务列表只显示尚未写入手机号、尚未完成且仍有 SIM 激活码的客户任务；`激活中`、`等待人工支付`、`失败` 的任务也可以重新选择。没有客户任务时，也可以直接选择“使用（可分配）”的 SIM 激活码创建新任务。
