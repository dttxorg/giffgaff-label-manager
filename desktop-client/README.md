...
4. 点击“领取下一个任务”。
5. 在“激活自动化预设”里填好固定英国地址、支付卡和“支付后自动解绑银行卡”。
6. 点击“打开并预填”，浏览器会打开 giffgaff 激活页。
7. 遇到邮箱验证码时点击“刷新验证码”，客户端会复制验证码；浏览器自动化还在运行时可点击“填入验证码”。
8. 客户端会继续完成密码、营销偏好、Pay as you go、£10、地址和支付卡填写。
9. 到支付页后，人工确认并点击 `Place order`，完成支付或 3DS。
10. 支付完成后保持浏览器打开；客户端会自动进入 `/profile/payment-details` 并移除保存的信用卡。也可以点击“自动解绑银行卡”手动触发。
11. 拿到手机号后填入客户端，点击“标记等待转 eSIM”或“标记完成”。
